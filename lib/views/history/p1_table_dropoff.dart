...
class P1TableDropoff extends StatefulWidget {
  const P1TableDropoff({Key? key}) : super(key: key);

  @override
  State<P1TableDropoff> createState() => _P1TableDropoffState();
}

class _P1TableDropoffState extends State<P1TableDropoff>
    with AutomaticKeepAliveClientMixin<P1TableDropoff> {
  late RealtimeChannel _channel;
  List cells = [];

  getP1TableData() async {
    showDialog(
        context: context,
    ...
    await DispatchService().getCompleteAllLotOne("lot1").then((res) {
      if (res != null) {
        lot1GetCompletedHistory(res);
        setState(() {
          cells = lot1GetCompletedHistory(res);
        });
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) async {
      ...
      _channel = SupabaseService()
          .liveUpdate(['LOT_1_HISTORY_UPDATE'], getP1TableData);
    });
  }
          ...
          for (var item in cells)
            item['isHistoryPickup'] != true
                ? TableRow(children: [
                    ...
                    Container(
                      color:
                          item['advanceTime'] > 10 && item['advanceTime'] <= 20
                              ? Colors.yellow[800]
                              : item['advanceTime'] > 20
                                  ? Colors.red
                                  : Colors.white,
                      child: Padding(
                        padding: const EdgeInsets.fromLTRB(0, 8, 0, 8),
                        child:
                            Center(child: Text(item['advanceTime'].toString())),
                      ),
                    ),
                    Padding(
                      padding: const EdgeInsets.fromLTRB(0, 8, 0, 8),
