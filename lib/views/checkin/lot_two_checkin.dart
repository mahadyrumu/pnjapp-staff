...
class LotTwoCheckIn extends StatefulWidget {
  const LotTwoCheckIn({Key? key}) : super(key: key);

  @override
  State<LotTwoCheckIn> createState() => _LotTwoCheckInState();
}

class _LotTwoCheckInState extends State<LotTwoCheckIn>
    with AutomaticKeepAliveClientMixin<LotTwoCheckIn> {
  ...
  void _getDropoffs() async {
    ...
    await DropoffService().getDropoffAllLotTwo().then((res) {
      if (res != null) {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) async {
      ...
      _getDropoffs();
      _channel =
          SupabaseService().liveUpdate(['LOT_2_DISPATCH_UPDATE'], _getDropoffs);
    });
  }
              ...
              TableRow(children: [
                ...
                Container(
                  color: Color(int.parse(
                      data['parkingStatusBackground'].replaceAll('#', '0xff'))),
                  child: Padding(
                    padding: const EdgeInsets.fromLTRB(0, 8, 0, 8),
                    child: Center(
                        child: Text(data['parkingStatus'] != null
                            ? data['parkingStatus'].toString()
                            : "--")),
                  ),
                ),
                Padding(
